...
const serverConfigs: ServerConfigs = {
  aws: {
    nginx: "2831", // Active for Nginx testing
    // ols: "2831"   // Commented out for Nginx testing
  },
};
...
test.describe("PHP Version Site Creation Tests", () => {
  test.beforeAll(async ({ browser }) => {
    ...
    // Initialize server
    server = new Server(page, serverConfigs.aws.nginx);
    await server.loadData();
    log(`AWS Nginx Server loaded with ID: ${serverConfigs.aws.nginx}`);
  });
